...
from similar_module import AttentionClassifier
...
class VisionTransformer(nn.Module):
    ...
    def __init__(
        ...
        # print(use_e_prompt)
        # print('prompt shape:')
        # if(use_e_prompt and use_e_prompt):
        #     print(type(self.g_prompt))
        #     print(type(self.e_prompt))
        #     print(self.g_prompt.shape)

        # <class 'torch.nn.parameter.Parameter'>
        # <class 'prompt.EPrompt'>
        # torch.Size([2, 2, 5, 12, 64]) prompt_shape_mark
            # print(self.e_prompt.shape) (3, 2, 30, 5, 12, 64)
        ...
        #TODO: Change similar module
        self.similar_module = AttentionClassifier(768,2)
    ...
    def init_weights(self, mode=''):
        ...
        head_bias = -math.log(2) if 'nlhb' in mode else 0. # self.num_classes*
    ...
    def forward_features(self, x, task_id=-1, cls_features=None, train=False):
                ...
                for i, block in enumerate(self.blocks):
                    if i in self.g_prompt_layer_idx:
                        if self.use_prefix_tune_for_g_prompt:
                            g_prompt_counter += 1
                            # Prefix tunning, [B, 2, g_prompt_length, num_heads, embed_dim // num_heads] prompt_shape_mark
        ...
        res['x'] = x
        # print('debug 1:')
        # print(x.shape) torch.Size([24, 197, 768])
    ...
    #     res['pre_logits'] = x
    #     # print('debug 2:')
    #     # print(x.shape) torch.Size([24, 768])

    #     x = self.fc_norm(x)
    #     # print('debug 3:')
    #     # print(x.shape) torch.Size([24, 768])

    #     # print(torch.max(x)) 0.7
    #     # print(torch.min(x)) -0.8
        
    #     res['logits'] = self.head(x)
    #     # print('debug 4:')
    #     # print(res['logits'].shape) torch.Size([24, 2])
        
    #     return res

    def forward_head(self, res, pre_logits: bool = False):
        x = res['x']
        if(type(pre_logits)==bool):
            if self.class_token and self.head_type == 'token':
                if self.prompt_pool:
                    x = x[:, self.total_prompt_len]
                    # print('debug 1:')
            ...
            res['pre_logits'] = x
            res['logits'] = self.similar_module(x,pre_logits)
        
        return res

    def forward(self, x, task_id=-1, cls_features=None, train=False, pre_logits=False):
        res = self.forward_features(x, task_id=task_id, cls_features=cls_features, train=train)
        # if(cls_features!=None):
        #     print('cls shape:')
        #     print(cls_features.shape) # torch.Size([8, 768])
        # else:
        #     print('cls None')
        # print(res['x'].shape) # torch.Size([8, 197, 768])
        res = self.forward_head(res, pre_logits) #TODO: classify head is a fc, change it
        # print(res['pre_logits'].shape) #torch.Size([8, 768])
        return res
