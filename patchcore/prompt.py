...
class EPrompt(nn.Module):
    ...
    def set_prompt(self,saved_prompt):
        self.prompt = nn.Parameter(saved_prompt)
    ...
    def forward(self, x_embed, prompt_mask=None, cls_features=None):
        ...
        if self.prompt_pool:
            ...
            if prompt_mask is not None:
                ...
                # print(idx)
                # print('prompt mask not None') True
            # idx = [[0]]*x_embed.shape[0]
            
            # print(idx) # 0*batchsize
            out['prompt_idx'] = idx
            # print('prompt idx:')
            # print(idx)
            if self.use_prefix_tune_for_e_prompt:
                # print('prompt shape:')
                # print(self.prompt.shape) #torch.Size([3, 2, 30=pool_size, 5, 12, 64]) # torch.Size([12, 2, 15, 12, 12, 64])
                batched_prompt_raw = self.prompt[:,:,idx]  # num_layers, B, top_k, length, C
                # print(idx)
                # print(batched_prompt_raw.shape) # torch.Size([3, 2, 16=batch_size, 1, 5, 12, 64]) # torch.Size([12, 2, 8, 1, 12, 12, 64])
                ...
                # print(batched_prompt.shape)# torch.Size([3, 16, 2, 5, 12, 64]) # torch.Size([12, 8, 2, 12, 12, 64])
            else:
        ...
        # print('prompt shape')
        # print(self.prompt.shape) #torch.Size([12, 2, 15, 12, 12, 64])
        # print('batch prompt shape')
        # print(batched_prompt.shape) #torch.Size([12, 8, 2, 12, 12, 64])
