...
from patchcore.prompt import EPrompt
from patchcore.attention import PreT_Attention
...
def augment_image(image, resize_shape, perlin_scale, anomaly_source_img):
        aug = randAugmenter()
        anomaly_img = np.zeros(resize_shape)
        perlin_scale = perlin_scale
        feat_rand = np.random.rand(28,28)
        feat_label = np.zeros(feat_rand.shape)
        feat_label[feat_rand>0.8] = 1 
        perlin_thr = np.repeat(np.repeat(feat_label,8,axis=0),8,axis=1)
        feat_label[feat_label<0.9] = -1
        perlin_thr = np.expand_dims(perlin_thr, axis=2)

        img_thr = anomaly_img.astype(np.float32) * perlin_thr / 255.0
        ...
        if np.sum(msk) == 0:
            has_anomaly=0
        return augmented_image, msk, int(has_anomaly), feat_label

def perlin_noise_tensor(image,probability):
    shape = image.shape
    anomaly_source_paths = sorted(glob.glob('./data/dtd/images/'+"/*/*.jpg"))
    target = torch.ones(image.shape[0],28,28)
    # mask_area = np.ones(shape)
    for i in range(shape[0]):
        randomN = np.random.random_sample(1)
        anomaly_source_idx = torch.randint(0, len(anomaly_source_paths), (1,)).item()
        anomaly_source_img = cv2.imread(anomaly_source_paths[anomaly_source_idx])
        anomaly_source_img = cv2.resize(anomaly_source_img, dsize=(shape[2], shape[3]))
        if(randomN<probability):
            tempTensor = image[0].squeeze(0).permute(1,2,0).cpu().numpy()
            tempTensor, mask, has_anomaly, feat_label = augment_image(tempTensor,[shape[2],shape[3],shape[1]],6, anomaly_source_img)
            tempTensor = torch.tensor(tempTensor).permute(2,0,1).unsqueeze(0)
            image[i,:,:,:] = tempTensor.to('cuda')
...
class VisionTransformer(nn.Module):
    ...
    def __init__(
            ...
            use_e_prompt=False, e_prompt_layer_idx=None, use_prefix_tune_for_e_prompt=False, same_key_value=False, prototype_size=5, feature_layer = [5]):
        """
        Args:
    ...
    def forward_features(self, x, task_id=-1, cls_features=None, train=False, query_task_id=None):
    ...
    def forward(self, x, task_id=-1, cls_features=None, train=False, pre_logits=False, label=None, image_path=None, query_task_id=None):
